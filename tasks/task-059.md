...
### 変更ファイル
- `playwright.config.ts` - Playwright設定を新規作成（モバイルビューポート、baseURL、タイムアウト設定）
- `tests/chat-quality.spec.ts` - チャット品質テスト6件を新規作成

### 実装内容
- Playwrightの設定ファイルを作成し、モバイルビューポートでのテスト実行を設定
- チャット品質テスト6件を実装:
  1. チャット画面の正常表示（ヘッダー・入力欄・送信ボタン・初期表示の確認）
  2. メッセージ送信→日本語回答の受信
  3. 期限質問に対する期限/書類関連の回答確認
  4. 対応方法の質問に電話を勧めない回答確認
  5. 差押予告通知に対する寄り添い+具体的アクション提示の確認
  6. 連続メッセージでのエラーなし確認
- 全6テスト pass（1.3分）
