...
### 変更ファイル
- `app/(main)/home/page.tsx` - `visibilitychange` イベントリスナーを追加し、画面復帰時にデータ再取得
- `app/(main)/documents/page.tsx` - 同様に `visibilitychange` イベントリスナーを追加

### 実装内容
- A案を採用: `document.addEventListener("visibilitychange", ...)` で `document.visibilityState === "visible"` 時にデータを再フェッチ
- ホーム画面・書類一覧の両方に適用
- useEffectのクリーンアップで `removeEventListener` を確実に実行
